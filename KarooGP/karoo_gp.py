...
if len(sys.argv) < 3 and (not no_input): # either no command line argument, or only a filename is provided
...
#++++++++++++++++++++++++++++++++++++++++++
	...
	if no_input:
		...
		kernel = 'r'
		tree_type = 'r'
		tree_depth_base = 10 # 10
		tree_depth_max = 10 # 10
		tree_depth_min = 19 # nodes not dep, 19
		tree_pop_max = 50 # 50
		gen_max = 50 # 50
		tourn_size = 3
		rep_rate = 0.0
		...
		br_rate = 0.05
		cr_rate = 0.9
		mut_rate = pt_rate + br_rate
		evolve_repro = 1
		evolve_point = int(float(pt_rate) * tree_pop_max)
		evolve_branch = int(float(br_rate) * tree_pop_max)
		evolve_cross = int(float(cr_rate) * tree_pop_max)
		filename = ''
		...
		print("Kernel mode: ", kernel)
		print("Tree type: ", tree_type)
		print("Tree dep init: ", tree_depth_base)
		print("Tree dep max: ", tree_depth_max)
		print("Minimum nodes: ", tree_depth_min)
		print("Pop: ", tree_pop_max)
		print("Gens: ", gen_max)
		print("Tour size: ", tourn_size)
		print("Mutation rate", mut_rate)
		print("Crossover rate", cr_rate)
...
for res in test_cases:
	for run_ind in range(runs):
		...
		gp.fx_karoo_gp(kernel, tree_type, tree_depth_base, tree_depth_max, tree_depth_min, tree_pop_max, gen_max, tourn_size, filename, evolve_repro, evolve_point, evolve_branch, evolve_cross, display, precision, swim, mode, seed , res)
