...
### changed by Award_of_Sky 11/12/2020
# nasty global vars
#resolution = [8, 8]
#num_points = np.prod(resolution)
min_domain = -5
max_domain = 5
...
class Base_GP(object):
	...
	def fx_karoo_gp(self, kernel, tree_type, tree_depth_base, tree_depth_max, tree_depth_min, tree_pop_max,
					gen_max, tourn_size, filename, evolve_repro, evolve_point, evolve_branch, evolve_cross, display, precision, swim, mode, seed, res):
		...
		self.print_final(time.time() - start)
		
		return

	def print_final(self, timetaken):
		_fp = os.getcwd() + '/files/runs/' + str(self.seed) + '_stats.csv'
		with open(_fp, mode='a', newline='') as file:
			fwriter = csv.writer(file)

			fwriter.writerow([str(self.seed)])
			fwriter.writerow([str(self.res)])
			fwriter.writerow([str(timetaken)])
	...
	def fx_data_load(self, filename):
		...
		if len(sys.argv) == 1: # load data from the default karoo_gp/files/ directory
			### changed by Award_of_Sky 11/12/2020
			# no need to load data from file (doing this in regression mode)
		...
		### changed by Award_of_Sky 11/12/2020 [START]
		# Generate domain instead of read from file
		data_x_x = np.tile(np.reshape(np.linspace(min_domain, max_domain, num=resolution[0]), (1, resolution[0])), (resolution[1], 1))
		data_x_y = np.tile(np.reshape(np.linspace(min_domain, max_domain, num=resolution[1]), (resolution[1], 1)), (1, resolution[0]))
		data_y = (1 / (1 + (1 / (data_x_x ** 4)))) + (1 / (1 + (1 / (data_x_y ** 4)))) # pagie polynomial
		data_x = np.stack([np.reshape(data_x_x, (num_points)), np.reshape(data_x_y, (num_points))], axis=1)
		#print("Before target")
		#print(data_y)
		data_y = np.reshape(data_y, (num_points))
		if resolution[0] < 10:
			print("[DEBUG]")
			...
			print("Data target")
			print(data_y)
		### changed by Award_of_Sky 11/12/2020 [END]
	...
	def print_stats(self, fits, tree):
		_fp = os.getcwd() + '/files/runs/' + str(self.seed) + '_stats.csv'
		with open(_fp, mode='a', newline='') as file:
			...
			fwriter.writerow([self.gen_id, fit_avg, fit_std, fit_best, tree])
			print(self.gen_id, ",", fit_avg,",", fit_std, ",", fit_best, ",", tree)
	...
	def fx_fitness_gym(self, population):
		...
		for tree_id in range(1, len(population)):
			...
			#if self.display not in ('s'): print ('\t\033[36mTree', population[tree_id][0][1], 'yields (sym):\033[1m', self.algo_sym, ', (srepr):\033[1m', srepr(self.algo_sym), '\033[0;0m')
			#if self.display not in ('s'): print ('\033[36m\033[1m', srepr(self.algo_sym), '\033[0;0m')
			...
			if self.display not in ('s'): print ('\t\033[36mTree', population[tree_id][0][1], 'yields (sym):\033[1m', self.algo_sym, ", with fitness:", fitness, '\033[0;0m')
			...
			if self.kernel == 'c': # display best fit Trees for the CLASSIFY kernel
				...
				if fitness <= fitness_best: # find the Tree with Minimum fitness score
					fitness_best = fitness # set best fitness score
					tree_best = self.algo_sym ###changed by Award_of_Sky 11/12/2020
					self.fittest_dict.update({tree_id:self.algo_sym}) # add to dictionary if fitness <= prior
		...
		self.print_stats(fits, tree_best)
		...
		###changed by Award_of_Sky 11/12/2020 [START]
		print("Current generation: ", self.gen_id)
		print("Best (fitness, tree): ", fitness_best, " | ", tree_best)
		###changed by Award_of_Sky 11/12/2020 [END]
		...
		with tf.Session(config=config) as sess:
			with sess.graph.device(self.tf_device):
				...
				if self.kernel == 'c': # CLASSIFY kernel
					...
					pairwise_fitness = tf.square(solution - result) # emulate RMSE
					### changed by Award_of_Sky 11/12/2020 [END]
				...
				### changed by Award_of_Sky 11/12/2020 [START]
				# tf.reduce_sum(pairwise_fitness)
				fitness = tf.sqrt(tf.reduce_mean(pairwise_fitness))
				### changed by Award_of_Sky 11/12/2020 [END]
				...
				#print ### changed by Award_of_Sky 11/12/2020
